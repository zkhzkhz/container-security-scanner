...
log_cmd "cat /etc/os-release 2>/dev/null | head -5"
local os_info=$(cat /etc/os-release 2>/dev/null | head -5)
if [ -n "$os_info" ]; then
...
# 忽略的挂载点
EXCLUDES=("/" "/dev" "/sys/fs/cgroup" "/etc/hosts" "/dev/shm" "/proc/acpi" "/proc/scsi" "/sys/firmware")

# 获取挂载点列表
MOUNTS=$(df -h | awk 'NR>1 {print $6}' | sort -u)
...
SCAN_PATHS=("/opt" "/opt/app" "/home" "/etc/nginx/cert" "/etc/nginx" "/var/log" "/var/www" "/app" "/data")
IGNORE_PATH=("/opt" "/home" "/var/log")
...
for path in "${SCAN_PATHS[@]}"; do
    ...
    local sensitive_files=$(find "$path" -type f \( -name '*.key' -o -name '*.pem' -o -name '*application.yml' -o -name '*application.yaml' \) 2>/dev/null | head -10)
    ...
    # 检查其他用户可写的文件
    log_cmd "find $path -type f -perm /007 2>/dev/null | head -10"
    local writable_files=$(find "$path" -type f -perm /007 2>/dev/null | head -10)
...
files="/etc/bashrc /etc/profile /root/.bashrc"
found_history_disable=false
...
TOP_OUT=$(ps -eo pid,ppid,user,comm --sort=pid | head -n 50)
echo "$TOP_OUT"
...
# 检查环境变量中的敏感信息
env_output=$(env 2>/dev/null)
sensitive_patterns="password passwd secret api_key token private_key access_key secret_key credential auth"
...
# 下载并安装gitleaks
arch=$(uname -m)
GITLEAKS_URL="https://gitee.com/opensourceway/sec_efficiency_tool/releases/download/1.0.0/gitleaks_8.27.0_linux_x64.tar.gz"
...
if which sudo >/dev/null 2>&1; then
    ...
    # 检查sudoers配置
    log_cmd "cat /etc/sudoers 2>/dev/null | grep -vE '^#|^$'"
    if [ -f /etc/sudoers ]; then
        local sudoers_content=$(cat /etc/sudoers 2>/dev/null | grep -vE '^\s*#|^\s*$')
        if [ -n "$sudoers_content" ]; then
            echo "$sudoers_content" | while read line; do
                log_detail "$line"
            done
        fi
    fi
    ...
    # 检查NOPASSWD配置
    log_cmd "grep -r 'NOPASSWD' /etc/sudoers /etc/sudoers.d 2>/dev/null"
    local nopasswd=$(grep -r 'NOPASSWD' /etc/sudoers /etc/sudoers.d 2>/dev/null)
    if [ -n "$nopasswd" ]; then
        log_warning "发现 NOPASSWD 配置:"
    ...
    # 检查sudo/wheel组成员
    log_cmd "getent group sudo wheel 2>/dev/null"
    local sudo_members=$(getent group sudo wheel 2>/dev/null)
    if [ -n "$sudo_members" ]; then
        log_info "sudo/wheel组成员:"
        echo "$sudo_members" | while read line; do
            log_detail "$line"
        done
    fi
else
...
for dir in "${path_dirs[@]}"; do
    ...
    find "$dir" -maxdepth 1 -type f -executable 2>/dev/null | while read file; do
        if [ -f "$file" ]; then
            ...
            # 检查其他用户写权限
            other_writable=${perms:8:1}
            group_writable=${perms:5:1}
...
if [ ${#writable_cmds[@]} -eq 0 ]; then
    log_success "PATH 中未发现对其他用户可写的命令文件"
fi

# ==================== 11. 网络端口与SSL检查 ====================
echo ""
echo "--- 11. 网络端口与 SSL 检查 ---"
log_cmd "netstat -tunlp 2>/dev/null || ss -tunlp"

NETSTAT_OUTPUT=$(netstat -tunlp 2>/dev/null || ss -tunlp 2>/dev/null)
echo "$NETSTAT_OUTPUT"
...
echo "$NETSTAT_OUTPUT" | while read -r line; do
    ...
    if [[ "$PORT" == "443" ]] || [[ "$PORT" == "8443" ]] || [[ "$PORT" == "9443" ]]; then
        ...
        if grep -q "CONNECTED" "$tmpfileSSL"; then
            ...
            # 提取协议和加密套件
            local ssl_info=$(grep "New," "$tmpfileSSL" | head -1)
            if [ -n "$ssl_info" ]; then
                log_detail "$ssl_info"
            fi

            # 检查证书信息
            local cert_info=$(openssl x509 -noout -dates -subject < "$tmpfileSSL" 2>/dev/null)
            if [ -n "$cert_info" ]; then
                log_detail "证书信息:"
                echo "$cert_info" | while read l; do
                    log_detail "  $l"
                done
            fi

            # 检查弱协议
            if grep -qE "TLSv1[^.]|SSLv" "$tmpfileSSL"; then
                log_warning "检测到不安全的协议版本"
            fi
        else
            log_detail "未检测到SSL"
        fi
...
DEBUG_TOOLS=$(find / -type f \( -name 'tcpdump' -o -name 'gdb' -o -name 'strace' -o -name 'nc' -o -name 'nmap' -o -name 'wireshark' -o -name 'netcat' \) 2>/dev/null | grep -v '/nmap/rpc\|/locale')
